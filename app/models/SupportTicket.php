...
class SupportTicket {
    ...
    private function ensureTable(): void {
        ...
        if (!in_array('related_order_id', $cols, true)) {
            $alter[] = 'ADD COLUMN related_order_id INT NULL AFTER created_by_user_id';
        }
    ...
    public function create(array $data): int {
        $pdo = $this->db->getConnection();
        $stmt = $pdo->prepare("INSERT INTO support_tickets (owner_id, customer_id, issue_type, subject, message, status, priority, source, created_by_user_id, related_order_id, related_order_number, attachment_path) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
        $stmt->execute([
            (int)$data['owner_id'],
    ...
    public function listByOwner(int $ownerId, ?string $status = null): array {
        $pdo = $this->db->getConnection();
        $sql = 'SELECT t.id, t.issue_type, t.subject, t.message, t.status, t.priority, t.source, t.attachment_path, t.created_at, c.company, c.contact_name, c.contact_email
    ...
    public function updateStatus(int $ownerId, int $id, string $status): bool {
        $allowed = ['pending', 'open', 'closed'];
        if (!in_array($status, $allowed, true)) {
            return false;
        }
        $pdo = $this->db->getConnection();
        $stmt = $pdo->prepare('UPDATE support_tickets SET status = ?, updated_at = NOW() WHERE owner_id = ? AND id = ?');
        $stmt->execute([$status, $ownerId, $id]);
