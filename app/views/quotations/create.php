...
<form id="quotationForm" method="POST" action="/?action=quotations&subaction=store" enctype="multipart/form-data">
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <div class="input-group">
                        <input class="form-control" name="customer" id="customer_input" value="<?= htmlspecialchars($prefill['customer'] ?? '') ?>" required/>
                ...
                <div class="col-md-3">
                    ...
                    <input class="form-control" name="quote_no" value="<?= (int)$nextNo ?>" required/>
                </div>
                <div class="col-md-3">
                ...
                <div class="col-md-3">
                    ...
                    <input type="date" class="form-control" name="valid_till" value="<?= date('Y-m-d', strtotime('+15 days')) ?>"/>
                </div>
                <div class="col-md-3">
                    <label class="form-label">Type</label>
                    <select class="form-select" name="type">
                        <?php $pt = $prefill['type'] ?? 'Quotation'; ?>
                        <option value="Quotation" <?= ($pt==='Quotation')? 'selected':'' ?>>Quotation</option>
                        <option value="Proforma" <?= ($pt==='Proforma')? 'selected':'' ?>>Proforma</option>
                    </select>
                </div>
                <div class="col-md-3">
...
function openAddCustomerInline(){
  ...
  document.getElementById('addCustomerTypeHidden').value = 'customer';
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  [tCust, tLead].forEach(el=> el.addEventListener('change', ()=>{ cpType = currentCpType(); searchContacts(); }));
  ...
  // Auto-fill when typing a company name directly in Customer field
...
// Try to auto-fill based on typed company name (Customers only)
...
function bindAddCustomerFormOnce(){
  ...
  try {
    ...
    document.getElementById('customer_input').value = companyName;
    
    // Optionally trigger auto-fill to get full details
    setTimeout(() => {
      tryAutoFillFromCustomerName();
    }, 100);
  } catch (err) {
